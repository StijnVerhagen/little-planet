...
export default {
  ...
  methods: {
    ...
    initPane() {
      ...
      pane.addInput(this.planetProps, "maxTrees", {
        min: 1,
        max: 750,
        step: 1,
      });
      pane.addInput(this.planetProps, "maxRocks", {
        min: 1,
        max: 750,
        step: 1,
      });
      ...
      folder.addInput(this.planetProps, "treeBodyColor1", {
        label: "BodyColor1",
      });
      folder.addInput(this.planetProps, "treeBodyColor2", {
        label: "BodyColor2",
      });
      folder.addInput(this.planetProps, "treeBodyColor3", {
        label: "BodyColor3",
      });
      folder.addInput(this.planetProps, "treeBodyColor4", {
        label: "BodyColor4",
      });
      folder.addInput(this.planetProps, "treeBodyColor5", {
        label: "BodyColor5",
      });

      folder = pane.addFolder({ title: "Other Colors", expanded: false });
      folder.addInput(this.planetProps, "rockColor", { label: "RockColor" });
      folder.addInput(this.planetProps, "groundColor", {
        label: "GroundColor",
      });
      folder.addInput(this.planetProps, "waterColor", { label: "WaterColor" });

      folder = pane.addFolder({ title: "Planet Config", expanded: false });
      folder.addInput(this.planetProps.noiseConf, "noiseF", {
        label: "Noise",
        min: 0.0001,
        max: 0.05,
      });
      folder.addInput(this.planetProps.noiseConf, "noiseD", {
        label: "Displacement",
        min: 0,
        max: 50,
        step: 1,
      });
      folder.addInput(this.planetProps.noiseConf, "noiseWaterTreshold", {
        label: "WaterTreshold",
        min: 0,
        max: 1,
      });
      folder.addInput(this.planetProps.noiseConf, "noiseWaterLevel", {
        label: "WaterLevel",
        min: 0,
        max: 1,
      });
      ...
      pane.addButton({ title: "Remove Trees" }).on("click", () => {
        this.$refs.planet.deleteTrees();
      });
      pane.addButton({ title: "Remove Rocks" }).on("click", () => {
        this.$refs.planet.deleteRocks();
      });
